 ...
 * Date:  2017/2/17
 */
public class DragDiootoView2 extends FrameLayout {
    ...
    public DragDiootoView2(Context context) {
        this(context, null);
    }

    public DragDiootoView2(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public DragDiootoView2(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void backToMin() {
        ...
        final int[] points = iDiooto.providerReleasePoint();
        ...
        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                float value = (float) animation.getAnimatedValue();
                min2NormalAndDrag2Min(value, points[2], mOriginTop, points[0], mOriginLeft, points[1], mOriginWidth, points[3], mOriginHeight);
            }
        });
        valueAnimator.setDuration(animationDuration).start();
        if (onReleaseListener != null) {
    ...
    public void notifySize(int width, int height, boolean showRightNow) {
        ...
        int[] size = iDiooto.providerEndSize();

        final int endLeft = (screenWidth - size[0]) / 2;
        final int endHeight = size[1];
        final int endWidth = size[0];
...
//        }
        ...
        if (showImmediately) {
            ...
            valueAnimator.addListener(new AnimatorListenerAdapter() {
                ...
                public void onAnimationEnd(Animator animation) {
                    isAnimating = false;
                    if (onShowFinishListener != null) {
                        onShowFinishListener.showFinish(DragDiootoView2.this, false);
                    }
                }
            });
            valueAnimator.setDuration(animationDuration).start();
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        ...
        if (iDiooto.needEvent(event, view)) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (Math.abs(mYDistanceTraveled) < touchSlop || (Math.abs(mYDistanceTraveled) > Math.abs(mYDistanceTraveled) && !isDrag)) {
                    if (!isMulitFinger && onClickListener != null) {
                        onClickListener.onClick(DragDiootoView2.this);
                    }
                    isMulitFinger = false;
                    if (isTouchPointInContentLayout(contentLayout, event)) {
                        break;
    ...
    private void changeImageViewToFitCenter() {
        iDiooto.changeImageViewToFitCenter();
    }

    private void changeImageViewToCenterCrop() {
        iDiooto.changeImageViewToCenterCrop();
    }
    ...
    public interface OnDragListener {
        void onDrag(DragDiootoView2 view, float moveX, float moveY);
    }

    public interface OnShowFinishListener {
        void showFinish(DragDiootoView2 dragDiootoView, boolean showImmediately);
    }
    ...
    public interface OnClickListener {
        void onClick(DragDiootoView2 dragDiootoView);
    }
